...
def create_speech_files(dialogue, output_dir='output_audio'):
    ...
    for i, item in enumerate(dialogue):
        ...
        print(f"  [{i+1}/{len(dialogue)}] {speaker}: {text[:30]}...")
...
def merge_audio_files(audio_files, output_filename='conversation_full.mp3', pause_duration=800):
    """
    複数の音声ファイルを1つに結合（セリフ間に間を入れる）
    """
    print("\n音声ファイルを結合中...")

    combined = AudioSegment.empty()
    pause = AudioSegment.silent(duration=pause_duration)  # ミリ秒
    ...
    print(f"  総時間: {len(combined) / 1000:.1f}秒")

def main():
    print("=" * 60)
    print("会話テキスト → 音声データ変換")
    print("=" * 60)

    # 会話をパース
    dialogue = parse_conversation(conversation)
    print(f"\n解析結果: {len(dialogue)}個のセリフを検出")
    ...
    print("完了!")
    print("=" * 60)
    print(f"個別ファイル: output_audio/ ディレクトリ内")
    print(f"結合ファイル: referee_meeting.mp3")
