...
const STORAGE_KEY_PLAYERS = 'player-management-a-players'
const STORAGE_KEY_REG = 'player-management-a-reg'
const STORAGE_KEY_APPROVAL = 'player-management-a-approval'
const STORAGE_KEY_ASSOC = 'player-management-a-assoc'
const STORAGE_KEY_PREF_ASSOC = 'prefecture-association'
const STORAGE_KEY_RELEASE = 'player-management-a-release'
...
const PlayerManagementA: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      // 一度だけ協会承認待ちをクリア
      const cleared = localStorage.getItem('assoc-cleared-a')
      if (!cleared) {
        localStorage.setItem(STORAGE_KEY_ASSOC, JSON.stringify([]))
        setAssocFiles([])
        localStorage.setItem('assoc-cleared-a', 'yes')
      }
    }
  ...
  return (
    <div style={{ padding: '24px', color: '#0f172a' }}>
      <h2 style={{ marginBottom: '12px', color: '#0b2545' }}>PROTHENTIAFC</h2>
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      ...
                      pushToPrefectureAssociation([p], 'PROTHENTIAFC')
                    }}
                    style={{
                      padding: '8px 12px',
                      borderRadius: '10px',
...
export default PlayerManagementA
